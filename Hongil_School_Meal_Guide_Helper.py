...
def check_meal(time, date):
    if time == '아침':
        ...
        try:
            ...
            meal_data = data[2].dish_name
        except:
            print()
            print('급식 자료가 존재하지 않습니다.')

    f = open(f"{date[0:4]}년{date[4:6]}월{date[6:8]}일 {time} 급식 식단 .txt", "w")
    f.write(meal_data)
    f.close()


print('---------------Made by busi---------------')
print('목포홍일고등학교 급식식단 안내 도우미입니다.')
